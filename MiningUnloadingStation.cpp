...
namespace unloading_station {
    unsigned long id = 0;
}

MiningUnloadingStation::MiningUnloadingStation() {
    id_ = ++unloading_station::id;
}
...
void MiningUnloadingStation::RunFrame(unsigned current_time) {
    ...
    frame_count_++;
    simulation_report_.efficiency =
        (frame_count_ - total_time_inactive_) / static_cast<double>(frame_count_);
}

void MiningUnloadingStation::Update(unsigned current_time) {
    if (!selected_mining_truck_) {
        total_time_inactive_++;
        return;
    }

    simulation_report_.total_time_unloading_helium++;
    unsigned last_state_change_timestamp =
        selected_mining_truck_->GetLastStateChangeTimestamp();
    if ((current_time - last_state_change_timestamp) >= kUnloadingTimeInSeconds) {
        UnloadMiningTruck(current_time);
    }
}

unsigned MiningUnloadingStation::GetTimeSinceLastStateChange(
    unsigned current_time, MiningTruck* current_truck) {
    unsigned time_since_last_state_change =
        current_time - current_truck->GetLastStateChangeTimestamp();
...
unsigned MiningUnloadingStation::GetCurrentWaitTime(unsigned current_time) {
    ...
    return wait_time +
        GetTimeSinceLastStateChange(current_time, selected_mining_truck_);
}
...
void MiningUnloadingStation::UnloadMiningTruck(unsigned current_time) {
    ...
    selected_mining_truck_->SetState(current_time,
        MiningTruckState::TRAVELLING_TO_MINING_SITE);
    ...
    selected_mining_truck_->SetState(current_time,
        MiningTruckState::UNLOADING_HELIUM);

    waiting_mining_trucks_.erase(waiting_mining_trucks_.begin());
    for (auto& waiting_mining_truck : waiting_mining_trucks_) {
        // Updating state with it's current state but an updated timestamp to
        // prevent trucks pre maturely exiting the queue
        waiting_mining_truck->SetState(current_time,
            MiningTruckState::WAITING_TO_UNLOAD);
    }
}

void MiningUnloadingStation::QueueMiningTruck(unsigned current_time,
    MiningTruck* mining_truck) {
    if (!selected_mining_truck_) {
