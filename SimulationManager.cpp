...
void SimulationManager::LoadTruckIntoUnloadingStation(
    unsigned current_time, MiningTruck* mining_truck) {
    // TODO: Implement cache to store time so we don't need to iteration over for
    // loop for each truck
    ...
    for (auto& mining_unloading_station : mining_unloading_stations_) {
        unsigned current_wait_time =
            mining_unloading_station.GetCurrentWaitTime(current_time);
    ...
    mining_unloading_stations_[selected_index].QueueMiningTruck(current_time,
        mining_truck);
}
...
SimulationStatus SimulationManager::RunSimulation() {
    ...
    unsigned current_simulation_time_in_hours = 0;

    while (current_simulation_time_in_seconds < kAllocatedSimulationTime) {
        if (current_simulation_time_in_seconds % timing::kSecondsPerHour == 0) {
            std::cout << std::format("Simulation Hour {}",
                current_simulation_time_in_hours++)
                << std::endl;
        }
...
void SimulationManager::PrintAnalysisReports(std::ostream& os) {
    for (const auto& truck : mining_trucks_) {
        os << std::format("Truck {} Analysis Report", truck.GetTruckId())
            << std::endl;
        os << truck.GetSimulationReport() << std::endl << std::endl;
    }

    for (const auto& mining_unloading_station : mining_unloading_stations_) {
        os << std::format("Unloading Station {} Analysis Report",
            mining_unloading_station.GetStationId())
            << std::endl;
        os << mining_unloading_station.GetSimulationReport() << std::endl
            << std::endl;
    }
}