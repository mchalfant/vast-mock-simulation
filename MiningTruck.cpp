...
MiningTruck::MiningTruck() {
    id_ = ++mining_truck::id;
    simulation_report_.shortest_mining_time_in_seconds = kMaximumLoadingTimeInSeconds;
    simulation_report_.longest_mining_time_in_seconds = kMinimumLoadingTimeInSeconds;
}
...
void MiningTruck::RunFrame(unsigned current_time) {
    ...
    frame_count_++;
    simulation_report_.efficiency = (frame_count_ - time_waiting_to_unload_) / static_cast<double>(frame_count_);
}
...
void MiningTruck::Update(unsigned current_time) {
   ...
   if(mining_truck_state_ == MiningTruckState::MINING_HELIUM){
      if((current_time - last_state_change_timestamp_) >= mining_time_duration_) {
         SetState(current_time, MiningTruckState::TRAVELLING_TO_UNLOADING_STATION);
      }
      return;
   }

   //Is it possible for future iterations to have different travel time? If so, this will need to be refactor. However, it meets current requirements.
...
void MiningTruck::CalculateMiningTime() {
   //Unnecessary error checking for initial simulation but good practice in case min/max loading time field becomes configurable as well
   ...
   double resolution = kMaximumLoadingTimeInSeconds - kMinimumLoadingTimeInSeconds;
   mining_time_duration_ = kMinimumLoadingTimeInSeconds + 
       static_cast<unsigned>(std::floor(
       resolution * (static_cast<double>(::rand()) / static_cast<double>(RAND_MAX))));

   //Not part of original requirement but good for debugging
   if (mining_time_duration_ > simulation_report_.longest_mining_time_in_seconds) {
       simulation_report_.longest_mining_time_in_seconds = mining_time_duration_;
   }
   if (mining_time_duration_ < simulation_report_.shortest_mining_time_in_seconds) {
   ...
   simulation_report_.average_mining_time_in_seconds = 
       simulation_report_.total_mining_time_in_seconds / simulation_report_.number_of_mine_site_visits;
}
