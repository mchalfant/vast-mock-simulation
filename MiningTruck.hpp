...
struct SimulationReport {
    ...
    friend std::ostream& operator<<(std::ostream& os, const SimulationReport& report){
        return os << std::format(
            "Work Efficiency: {}%\nShortest Mining Time: {} seconds\n"
            "Longest Mining Time: {} seconds\nAverage Mining Time: {} seconds\n"
            "Total Mining Time: {} seconds\nMine Site Visits: {}", 
            report.efficiency * 100.0, report.shortest_mining_time_in_seconds, 
            report.longest_mining_time_in_seconds, report.average_mining_time_in_seconds,
            report.total_mining_time_in_seconds, report.number_of_mine_site_visits) << std::endl;
    }
};

//TODO: In future iterations of this simulation would we need different mining trucks?
//We would add a virtual destructor if we want to have specialized trucks inheriting from this class.

class MiningTruck {
    public:
       MiningTruck();

       MiningTruckState GetState() const {
          return mining_truck_state_;
       }
       ...
       unsigned long GetTruckId() const {
           return id_;
       }
    ...
    private:
       ...
       //TODO: If we have different types of truck will they have different loading times?
       static const unsigned kMinimumLoadingTimeInSeconds = 1 * timing::kSecondsPerHour;
       static const unsigned kMaximumLoadingTimeInSeconds = 5 * timing::kSecondsPerHour;
       ...
       SimulationReport simulation_report_;
};